...
describe('subscription: transfers', () => {
    it('transfers', (done) => {
        ...
        const sub = web3.eth.subscribe('transfers', { pos: '0x0009c5f530fa412e137ce153c53be953b7d7362482b855fce5572cb228879880' })
        .on('data', (data) => {
            if (data) {
                emitData = true
                try {
...
describe('subscription: contract', () => {
    ...
    it('contract subscription with multiple topic filter should throw error', (done) => {
        ...
        try {
            ...
            expect(() => { throw err || 'no error' }).to.throw('[meterify] Array filter option is not supported in meter, must be null or bytes32 string')
            done()
        }
